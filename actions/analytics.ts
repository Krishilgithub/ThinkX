...
export async function getAnalyticsOverview() {
  ...
  const totalViews = courses.reduce(
    (acc, c) => acc + (c.analytics?.totalViews || 0),
    0,
  );
  ...
  const engagementSum = courses.reduce(
    (acc, c) => acc + (c.analytics?.completionRate || 0),
    0,
  );

  const measuredCourses = courses.filter((c) => c.analytics !== null).length;
  const avgEngagement =
  ...
  // In a real app, we would have a ViewLog table { date, count }
  const viewsOverTime = [
  ...
  return {
    ...
    avgEngagement,
    completionRate: 78, // Hardcoded global average for now
    viewsOverTime,
    retentionData,
    quizData,
    engagementWeek,
